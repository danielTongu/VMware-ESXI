...
function Invoke-PowerOperation {
    ...
    try {
        ...
        # Execute operation with state validation
        $successCount = 0
        $ignoredCount = 0
        $failedCount = 0
        $operationResults = @()
        ...
        foreach ($vmName in $targetVMs) {
            try {
                ...
                # Validate VM state before operation
                $ignoreReason = $null
                switch ($Operation) {
                    'On' {
                        if ($vm.PowerState -eq 'PoweredOn') {
                            $ignoreReason = "VM is already powered on"
                        }
                    }
                    'Off' {
                        if ($vm.PowerState -eq 'PoweredOff') {
                            $ignoreReason = "VM is already powered off"
                        }
                    }
                    'Restart' {
                        if ($vm.PowerState -eq 'PoweredOff') {
                            $ignoreReason = "VM is powered off (cannot restart)"
                        }
                    }
                }

                if ($ignoreReason) {
                    $operationResults += "IGNORED: $vmName - $ignoreReason"
                    $ignoredCount++
                    continue
                }
                ...
                switch ($Operation) {
                    'On'     { 
                        $task = $vm | Start-VM -Confirm:$false -ErrorAction Stop -RunAsync
                        $task | Wait-Task -ErrorAction Stop | Out-Null
                    }
                    'Off'    { 
                        $task = $vm | Stop-VM -Confirm:$false -Kill:$true -ErrorAction Stop -RunAsync
                        $task | Wait-Task -ErrorAction Stop | Out-Null
                    }
                    'Restart' { 
                        $task = $vm | Restart-VM -Confirm:$false -Kill:$true -ErrorAction Stop -RunAsync
                        $task | Wait-Task -ErrorAction Stop | Out-Null
                    }
                }
                
                $operationResults += "SUCCESS: $vmName - $Operation completed"
                $successCount++
            }
            catch {
                $errorMsg = $_.Exception.Message -replace "`n", " " -replace "`r", " "
                $operationResults += "FAILED: $vmName - $errorMsg"
                $failedCount++
        ...
        $statusMessage = @(
            "Operation completed:",
            "$successCount succeeded, $ignoredCount ignored, $failedCount failed"
        ) -join " "
        
        # Determine message type
        $messageType = if ($failedCount -eq 0) { 'Success' } else { 'Warning' }
        Set-StatusMessage -Refs $Refs -Message $statusMessage -Type $messageType
        
        # Show summary of ignored/failed operations if any
        if ($ignoredCount -gt 0 -or $failedCount -gt 0) {
            $details = $operationResults -join "`n"
